...
export default class ShrimpoVotingCategoriesTable extends Component<ShrimpoVotingCategoriesTableArgs> {<template>{{#if this.session.isAuthenticated}}
  <div>
  ...
  {{/if}}
  {{#each-in this.votes key="score" as |name vote|}}
    <div class="shrimpo-score-picker mb-2">
      <h3 class="text-lg">{{name}}</h3>
      <input type="range" min="1" max="6" value={{vote.score}} {{on "change" (fn this.setScore name)}}>
      <div class="flex">
        <img width="48" src="{{emojiPath vote.emoji}}">
        <img class={{if (gte vote.score 2) "opacity-100" "opacity-50"}} width="48" src="{{emojiPath vote.emoji}}">
        <img class={{if (gte vote.score 3) "opacity-100" "opacity-50"}} width="48" src="{{emojiPath vote.emoji}}">
        <img class={{if (gte vote.score 4) "opacity-100" "opacity-50"}} width="48" src="{{emojiPath vote.emoji}}">
        <img class={{if (gte vote.score 5) "opacity-100" "opacity-50"}} width="48" src="{{emojiPath vote.emoji}}">
        <img class={{if (gte vote.score 6) "opacity-100" "opacity-50"}} width="48" src="{{emojiPath vote.emoji}}">
      </div>
    </div>
  {{/each-in}}
